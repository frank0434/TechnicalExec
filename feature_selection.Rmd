...
```{r, fig.width=8}
product_type <- data[, sum(Item_Outlet_Sales), by = .(Item_Type)]
product_type %>% 
  ggplot(aes(fct_reorder(Item_Type, V1),V1))+
...
popular_store  %>% 
  ...
  geom_text(aes(label=scales::comma(V1)), hjust=0, nudge_y=5000) 
```
## Why store 10 and 19 are so small? 
```{r}
data[Outlet_Type %like% 3 ]
data[Outlet_Identifier %in% c("OUT010", "OUT019")]
unique(data[,.(Outlet_Identifier, Outlet_Establishment_Year)]) %>% 
  ggplot(aes(forcats::fct_reorder( Outlet_Identifier, Outlet_Establishment_Year),Outlet_Establishment_Year)) +
  geom_point()+
  coord_flip()+
  scale_x_continuous()
```
